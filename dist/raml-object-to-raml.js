    ...
    Object.keys(resourceTypeMap).forEach(function (type) {
      ...
      Object.keys(resourceType).forEach(function (key) {
        ...
        if (METHOD_KEY_REGEXP.test(key)) {
          ...
          if (value && is.array(value.is)) {
            child[key].is = value.is;
          }
        }
...
/**
 * Check if it is a RAML include
 * @param {String} str
 */
var stringIsInclude = function (str) {
  return str.substring(0, '!include'.length) === '!include';
}
...
var stringifyProperty = function (prefix, value, level, opts) {
  ...
  if (is.string(value) && !stringWithinLength(value, maxLength, opts) && !stringIsInclude(value)) {
